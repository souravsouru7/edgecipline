    ...
    <div style={{ padding: "0 16px 60px", display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ background: C.card, borderRadius: 20, border: `1px solid ${C.border}`, padding: "22px 20px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
          <div><div style={{ fontSize: 18, fontWeight: 800, color: C.primary }}>{stats.totalTrades ?? 0}</div><div style={{ fontSize: "var(--fs-2xs)", color: C.muted }}>total trades</div></div>
          <div><div style={{ fontSize: 18, fontWeight: 800, color: C.primary }}>{stats.tradesWithRules ?? 0}</div><div style={{ fontSize: "var(--fs-2xs)", color: C.muted }}>with rules</div></div>
          <div><div style={{ fontSize: 18, fontWeight: 800, color: C.primary }}>{stats.uniqueRules ?? 0}</div><div style={{ fontSize: "var(--fs-2xs)", color: C.muted }}>rules tracked</div></div>
            ...
            {rulesForChart.map((r, i) => {
              ...
              return (
                <div key={i}>
                  ...
                  <div style={{ fontSize: "var(--fs-2xs)", color: C.muted, marginTop: 3 }}>Followed {r.timesFollowed}× · Skipped {r.timesBroken}×</div>
                </div>
              );
            })}
          </div>
...
<R.ResponsiveContainer width="100%" height="100%">
              <R.LineChart data={timelineData}>
                ...
                <R.ReferenceLine y={70} stroke="#CBD5E1" strokeDasharray="4 4" />
          ...
          <div style={{ display: "flex", gap: 16, marginTop: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}><div style={{ width: 16, height: 3, background: C.green, borderRadius: 2 }} /><span style={{ fontSize: "var(--fs-2xs)", color: C.muted }}>Follow rate</span></div>
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}><div style={{ width: 16, height: 3, background: C.blue, borderRadius: 2 }} /><span style={{ fontSize: "var(--fs-2xs)", color: C.muted }}>Win rate</span></div>
          </div>
        </div>
      )}
      ...
      {Array.isArray(data.psychologyCorrelation?.bySetupRange) && data.psychologyCorrelation.bySetupRange.length > 0 && (
        <div style={{ background: C.card, borderRadius: 16, border: `1px solid ${C.border}`, padding: "18px 16px" }}>
          ...
          <div style={{ height: 130 }}>
            <LazyRecharts>{(R) => (
<R.ResponsiveContainer width="100%" height="100%">
              <R.BarChart data={data.psychologyCorrelation.bySetupRange.map(r => ({ name: `${r.range} (n=${r.count})`, pnl: r.avgPnL ?? 0, count: r.count }))}>
                <R.CartesianGrid stroke="#F1F5F9" strokeDasharray="3 3" vertical={false} />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {strongestRule && (
              <div style={{ padding: "12px 14px", borderRadius: 12, background: "#F0FDF4", border: "1px solid #0D9E6E22" }}>
                ...
                <div style={{ fontSize: 11, color: C.muted }}>You follow this {strongestRule.compliancePct}% of the time ({strongestRule.timesFollowed || 0} of {ruleSample(strongestRule)} logged) — your strongest habit.</div>
              </div>
            )}
