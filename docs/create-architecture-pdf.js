...
const input = path.join(__dirname, "technical-architecture-document.md");
const output = path.join(__dirname, "edgecipline-technical-architecture-document.pdf");
...
page.push({ text: "Technical Architecture Document", size: 26, leading: 36, indent: 0 });
page.push({ text: "SaaS architecture, database schema, project structure, and configuration", size: 12, leading: 22, indent: 0 });
...
for (const rawLine of lines) {
  ...
  if (/^\|.*\|$/.test(line.trim())) {
    flushParagraph();
    addWrapped(line.replace(/\|/g, "  "), { size: 8, leading: 11, max: 105, indent: 8 });
    continue;
  }
...
for (let p = 0; p < pages.length; p++) {
  ...
  stream += `/FR 8 Tf\n1 0 0 1 62 34 Tm\n(${sanitize(`Edgecipline Architecture | ${p + 1}`)}) Tj\n`;
  stream += "ET\n";
