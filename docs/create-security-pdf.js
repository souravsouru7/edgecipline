...
const input = path.join(__dirname, "security-and-access-document.md");
const output = path.join(__dirname, "edgecipline-security-and-access-document.pdf");
...
page.push({ text: "Security And Access Document", size: 26, leading: 36, indent: 0 });
page.push({ text: "Authentication, roles, row-level security, errors, and launch edge cases", size: 12, leading: 22, indent: 0 });
...
for (let p = 0; p < pages.length; p++) {
  ...
  stream += `/FR 8 Tf\n1 0 0 1 62 34 Tm\n(${sanitize(`Edgecipline Security | ${p + 1}`)}) Tj\n`;
  stream += "ET\n";
