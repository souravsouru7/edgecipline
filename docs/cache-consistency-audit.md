...
All user trade mutations advance `trade_version:{userId}` through `invalidateTradeCaches`.
Trade-dependent Redis keys include that version, so old values become unreachable and expire by TTL.
...
- Fixed: frontend mutations now invalidate dashboard, analytics, DNA, timeline, discipline, patterns, AI Coach, reports, and trade queries together.
- Fixed: dashboard and analytics React Query stale windows were reduced to zero and now refetch on focus/reconnect.
...
All trade lifecycle events must call:
